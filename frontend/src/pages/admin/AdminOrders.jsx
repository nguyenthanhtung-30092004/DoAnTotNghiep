...
const getPagination = (data) => {
  ...
  return {
    ...
    totalPages:
      pagination?.totalPages || pagination?.totalPage || pagination?.pages || 1,
  };
};
...
const AdminOrders = () => {
  ...
  const counts = useMemo(() => {
    return {
      ...
      pending: orders.filter((order) => order.orderStatus === ORDER_STATUS.PENDING)
        .length,
      shipping: orders.filter(
        (order) => order.orderStatus === ORDER_STATUS.SHIPPING,
      ).length,
      paid: orders.filter((order) => order.paymentStatus === PAYMENT_STATUS.PAID)
        .length,
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="space-y-1">
                          ...
                          <span
                            className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${currentPaymentStatus.className}`}
                          >
