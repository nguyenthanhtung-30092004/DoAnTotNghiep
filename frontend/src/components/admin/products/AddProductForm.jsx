  ...
  const validateForm = () => {
    ...
    for (const variant of variants) {
      ...
      for (const item of variant.sizes) {
        ...
        if (item.price === "" || item.price === undefined || item.stock === "") {
        ...
        if (
          ...
          toast.error("Giá, giá khuyến mãi và tồn kho phải là số hợp lệ");
          return false;
        }

        if (price < 0 || salePrice < 0 || stock < 0) {
          toast.error("Giá, giá khuyến mãi và tồn kho không được âm");
          return false;
        }
        ...
        if (seenSkus.has(normalizedSku)) {
          toast.error(`SKU ${item.sku.trim()} bị trùng trong form`);
          return false;
        }
